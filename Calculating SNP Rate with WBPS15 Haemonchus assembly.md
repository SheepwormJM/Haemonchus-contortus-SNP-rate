...
Using a transcriptome annotation generated by Dr Steve Doyle, WSI, 22nd April 2020 WBPS15

Note that script ```./list.join2List.pl``` was kindly provided by Dr. Thomas Otto, University of Glasgow. 
...
```
	...
	#Great, so now have, for UGA MAPQ20, all the sites with a SNP allele different to the reference sequence with an allele frequency of >0.4.
